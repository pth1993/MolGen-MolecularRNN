...
class NodeRNN(nn.Module):
    def __init__(self, input_size_adj, input_size_node, emb_size, hidden_rnn_size, num_layers, hidden_header_size,
    ...
    def forward(self, x_adj, x_node, x_len):
        ...
        outputs = torch.zeros([h2.shape[0], self.seq_len, len(ATOM_IDX)], device=device)
        for i in range(self.seq_len):
            y = self.header(h2[:, i])
            outputs[:, i, ] = y

        return outputs, torch.stack([h1, h2], dim=1)


class EdgeRNN(nn.Module):
    def __init__(self, input_size, emb_size, hidden_rnn_size, num_layers, hidden_header_size, out_features,
                 seq_len, dr_rate=0.2):
...
if __name__ == "__main__":
    model = NodeRNN(input_size_adj=12, input_size_node=9, emb_size=64, hidden_rnn_size=128, num_layers=2,
