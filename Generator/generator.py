...
def sample_mol(node_model, edge_model):
    with torch.no_grad():
        ...
        S_ij = torch.zeros([1, MAX_NODE, MAX_TIMESTEP], dtype=torch.long).to(device)
        ...
        while C[0, i] != ATOM_IDX["E"] and i < MAX_NODE-1:
            ...
            X[i+1, node_ind] = 1
            if i < MAX_TIMESTEP:
                S_ij[0, i+1, MAX_TIMESTEP-i-1:] = torch.tensor(edge_ind[MAX_TIMESTEP-i-1:])
                S_i[0, i+1, MAX_TIMESTEP-i-1:] = torch.tensor(edge_ind_exh[MAX_TIMESTEP - i - 1:])
                for j in range(i):
                    if edge_ind_exh[j] != 0:
                        A[i+1, i-j-1, edge_ind_exh[j]] = 1
                        A[i-j-1, i+1, edge_ind_exh[j]] = 1
            else:
                S_ij[0, i+1, ] = torch.tensor(edge_ind)
...
if __name__ == "__main__":
    node_model = NodeRNN(input_size_adj=MAX_TIMESTEP, input_size_node=len(ATOM_IDX), emb_size=64, hidden_rnn_size=128,
                         num_layers=2, hidden_header_size=64, out_features=len(ATOM_IDX),
                         seq_len=MAX_NODE).to(device)
    edge_model = EdgeRNN(input_size=len(BOND_IDX), emb_size=128, hidden_rnn_size=128, num_layers=2,
                         hidden_header_size=64, out_features=len(BOND_IDX), seq_len=MAX_TIMESTEP).to(device)
    node_model.load_state_dict(torch.load("data/model/NodeRNN-ep10.pth"))
    edge_model.load_state_dict(torch.load("data/model/EdgeRNN-ep10.pth"))

    smiles_list = sample_mol_list(node_model, edge_model, num_sample=100)
